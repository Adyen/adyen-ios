...
open class BaseFormValuePickerItemView<T: CustomStringConvertible>: FormValueItemView<BaseFormValuePickerItem<T>>,
    ...
    public required init(item: BaseFormValuePickerItem<T>) {
        ...
        initialize()
        didSelection(selectionValue: item.value)
    }
    ...
    private func didSelection(selectionValue: BasePickerElement<T>) {
        self.item.value = selectionValue
        updateSelection()
