...
final class FormCardNumberItem: FormTextItem, Observer {
    ...
    @Observable("") var binValue: String
    
    /// nitial brand set after detection before any user interaction
    @Observable(nil) private(set) var initialBrand: CardBrand?
    
    /// Brand selected in dual branded cards, set after user selection.
    @Observable(nil) private(set) var selectedDualBrand: CardBrand?
    
    /// Detected brand logo(s) for the entered bin.
    @Observable([]) private(set) var detectedBrandLogos: [FormCardLogosItem.CardTypeLogo]
    
    /// Determines whether the item is currently the focused one (first responder).
    @Observable(false) var isActive
