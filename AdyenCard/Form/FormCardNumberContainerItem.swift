...
final class FormCardNumberContainerItem: FormItem, Observer {
...
final class FormCardLogosItem: FormItem, Hidable {
    ...
    var isHidden: Observable<Bool> = Observable(false)
    
    var identifier: String?
    
    var subitems: [FormItem] = []
    
    let cardLogos: [CardTypeLogo]
    
    let style: FormTextItemStyle
    
    /// Observable property to update the owner view's alpha.
    @Observable(1) var alpha: CGFloat
