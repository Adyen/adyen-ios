...
class SEPADirectDebitComponentTests: XCTestCase {
    ...
    func testUIConfiguration() {
        ...
        navigationStyle.barTintColor = .green
        navigationStyle.tintColor = .white
        navigationStyle.backgroundColor = .cyan

        let sepaPaymentMethod = SEPADirectDebitPaymentMethod(type: "bcmc", name: "Test name")
        let sut = SEPADirectDebitComponent(paymentMethod: sepaPaymentMethod, style: sepaComponentStyle, navigationStyle: navigationStyle)

        UIApplication.shared.keyWindow?.rootViewController = sut.viewController

        let expectation = XCTestExpectation(description: "Dummy Expectation")
        DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + .seconds(1)) {
            let nameItemView: FormTextItemView? = sut.viewController.view.findView(with: "Adyen.SEPADirectDebitComponent.nameItem")
            ...
            let ibanItemView: FormTextItemView? = sut.viewController.view.findView(with: "Adyen.SEPADirectDebitComponent.ibanItem")
            ...
            XCTAssertEqual((sut.viewController as? ComponentViewController)?.rootViewController.navigationController?.navigationBar.tintColor, .green)
            XCTAssertEqual((sut.viewController as? ComponentViewController)?.rootViewController.navigationController?.view.tintColor, .white)

            XCTAssertEqual(sut.viewController.view.backgroundColor, .cyan)
            XCTAssertEqual(((sut.viewController as? ComponentViewController)?.rootViewController as? FormViewController)?.view.backgroundColor, .red)
