...
internal struct BillingAddressModeSettingsView: View {
    @Binding internal var billingAddress: BillingAddressModeDemoSetting
        ...
        List {
            ...
            if viewModel.showsSupportedCountryCodes {
                SelectableDisclosureSection(
                    ...
                    rowTitle: { String("\($0) - \(viewModel.countryName(for: $0))") },
...
private struct BillingAddressModeSettingsPreviewContainer: View {
    @State private var billingAddress: BillingAddressModeDemoSetting = .full(
