...
internal class PreselectedPaymentMethodViewController: UIViewController {
    ...
    private var viewModel: PreselectedPaymentMethodViewModelProtocol
    ...
    // Call this when any subview resizes itself. That will allow this screen to redraw and then we can compute the dynamic heigh tof the screen.
    ...
    private lazy var primaryButton: FormButton = {
        ...
        button.leadingImage =
            (UIImage(named: "custom-lock") ?? UIImage(systemName: "lock"))?.withRenderingMode(.alwaysTemplate)
        return button
    }()
