...
extension CardComponentTests {
    ...
    func testUC10_PayButton_ValidatesAllFieldsSimultaneously() {
        // Given - form with multiple fields containing invalid values
        let (cardViewController, items, delegate) = setupCardFormWithInvalidFields()
        
        let fields: [(item: any FormValidatableValueItem, name: String)] = [
            (items.numberContainerItem.numberItem, "Card number"),
            (items.expiryDateItem, "Expiry date"),
            (items.securityCodeItem, "Security code"),
            (items.holderNameItem, "Holder name")
        ]
        
        assertNoValidationErrors(for: fields)
        ...
        assertValidationErrors(for: fields, shown: true, context: "after Pay button tap")
    ...
    private func setupCardFormWithInvalidFields() -> (
        cardViewController: CardViewController,
        items: CardViewController.ItemsProvider,
        delegate: CardComponentDelegateMock
    ) {
        var configuration = CardComponentConfiguration()
        configuration.showsHolderNameField = true
        ...
        let delegate = CardComponentDelegateMock()
        sut.delegate = delegate
        ...
        return (cardViewController, items, delegate)
    }
    
    private func assertNoValidationErrors(
        for fields: [(item: any FormValidatableValueItem, name: String)],
        file: StaticString = #file,
        line: UInt = #line
    ) {
        for field in fields {
            XCTAssertFalse(
                field.item.validationState.shouldShowError,
                "\(field.name) error should not be shown initially",
                file: file,
                line: line
            )
        }
    ...
    private func assertValidationErrors(
        for fields: [(item: any FormValidatableValueItem, name: String)],
        shown: Bool,
        context: String,
        file: StaticString = #file,
        ...
        for field in fields {
            XCTAssertEqual(
                field.item.validationState.shouldShowError,
                shown,
                "\(field.name) should \(shown ? "show" : "hide") validation error \(context)",
                file: file,
                line: line
            )
        }
