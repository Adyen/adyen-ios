...
final class CheckoutTests: XCTestCase {
    ...
    // MARK: - drop-in creation
    
    func test_createDropIn_withSession_shouldAssignCheckoutDelegates() throws {
        ...
        XCTAssertTrue(dropIn.partialPaymentDelegate === sut)
        XCTAssertTrue(dropIn.storedPaymentMethodsDelegate === sut)
        XCTAssertTrue(dropIn.configuration.paymentMethodsList.allowDisablingStoredPaymentMethods)
    }
    
    func test_createDropIn_withPaymentMethods_shouldAssignCheckoutDelegateOnly() throws {
        let sut = Checkout(
            configuration: configuration,
            paymentMethods: paymentMethods,
            adyenContext: Dummy.context,
        ...
        XCTAssertTrue(dropIn.delegate === sut)
        XCTAssertNil(dropIn.partialPaymentDelegate)
        XCTAssertNil(dropIn.storedPaymentMethodsDelegate)
    }
    
    // MARK: - drop-in delegate
    
    func test_didSubmitFromDropIn_withSessionAndMissingOnSubmit_shouldPerformSubmit() async throws {
        ...
        sut.didSubmit(paymentData, from: PaymentComponentMock(paymentMethod: blik), in: DropInComponentMock())
    ...
    func test_didSubmitFromDropIn_withActionResult_shouldHandleActionInDropIn() async throws {
        let actionExpectation = expectation(description: "Action handled")
        let blik = try XCTUnwrap(paymentMethods.paymentMethod(ofType: BLIKPaymentMethod.self))
        let paymentData = PaymentComponentData(
            paymentMethodDetails: BLIKDetails(paymentMethod: blik, blikCode: "code"),
            amount: nil,
        ...
        let action = try RedirectAction(url: XCTUnwrap(URL(string: "https://adyen.com")), paymentData: "paymentData")
        let session = makeSessionMock()
        session.performSubmitResult = .success(.action(.redirect(action)))
        let dropIn = DropInComponentMock()
        dropIn.onHandleAction = { handledAction in
            if case .redirect = handledAction {
        ...
        sut.didSubmit(paymentData, from: PaymentComponentMock(paymentMethod: blik), in: dropIn)
        await fulfillment(of: [actionExpectation], timeout: 1)
        
        XCTAssertTrue(session.performSubmitCalled)
    }
    
    func test_didSubmitFromDropIn_withPartialPayment_shouldReloadDropIn() async throws {
        let reloadExpectation = expectation(description: "Drop-in reloaded")
        let blik = try XCTUnwrap(paymentMethods.paymentMethod(ofType: BLIKPaymentMethod.self))
        let paymentData = PaymentComponentData(
            paymentMethodDetails: BLIKDetails(paymentMethod: blik, blikCode: "code"),
            amount: nil,
        ...
        let order = PartialPaymentOrder(
            ...
            remainingAmount: Amount(value: 500, currencyCode: "EUR"),
            expiresAt: Date()
        )
        let session = makeSessionMock()
        session.performSubmitResult = .success(.partialPayment(PartialPayment(order: order, paymentMethods: paymentMethods)))
        let dropIn = DropInComponentMock()
        dropIn.onReload = { reloadedOrder, _ in
    ...
    func test_didProvideFromDropIn_withSessionAndMissingOnAdditionalDetails_shouldPerformAdditionalDetails() async {
        let onCompleteExpectation = expectation(description: "onComplete called")
        ...
        sut.didProvide(actionData, from: ActionComponentMock(), in: DropInComponentMock())
        await fulfillment(of: [onCompleteExpectation], timeout: 1)
...
@MainActor
private final class DropInComponentMock: AnyDropInComponent, ActionHandlingComponent, Localizable {
    internal var context: AdyenContext = Dummy.context
    internal var localizationParameters: LocalizationParameters?
    internal let viewController = UIViewController()
    internal weak var delegate: DropInComponentDelegate?
    internal var onHandleAction: ((Action) -> Void)?
    internal var onReload: ((PartialPaymentOrder, PaymentMethods) -> Void)?
    
    internal func handle(_ action: Action) {
        onHandleAction?(action)
    }
    
    internal func reload(with order: PartialPaymentOrder, _ paymentMethods: PaymentMethods) throws {
        onReload?(order, paymentMethods)
    }
}
