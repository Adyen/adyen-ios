...
class LogoURLProviderTests: XCTestCase {
    ...
    func testSize() {
        let paymentMethod = try! Coder.decode(creditCardDictionary) as CardPaymentMethod
        let logo = LogoURLProvider.logoURL(for: paymentMethod, environment: .test, size: .large)
        ...
        let logo2 = LogoURLProvider.logoURL(for: paymentMethod, environment: .test, size: .medium)
        ...
        let logo3 = LogoURLProvider.logoURL(for: paymentMethod, environment: .test, size: .small)
