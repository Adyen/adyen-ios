...
class ThreeDS2CompactActionHandlerTests: XCTestCase {
    ...
    override func setUp() {
        ...
        let fingerprintToken = ""
    ...
    func testSettingThreeDSRequestorAppURL() {
        let sut = ThreeDS2CompactActionHandler(adyenContext: Dummy.adyenContext, appearanceConfiguration: ADYAppearanceConfiguration())
    ...
    func testWrappedComponent() {
        let sut = ThreeDS2CompactActionHandler(adyenContext: Dummy.adyenContext, appearanceConfiguration: ADYAppearanceConfiguration())
        XCTAssertEqual(sut.wrappedComponent.adyenContext.apiContext.clientKey, Dummy.context.clientKey)
        XCTAssertEqual(sut.wrappedComponent.adyenContext.apiContext.environment.baseURL, Dummy.context.environment.baseURL)
    ...
    func testFingerprintFlowInvalidFingerprintToken() throws {
        ...
        let sut = ThreeDS2CompactActionHandler(adyenContext: Dummy.adyenContext, fingerprintSubmitter: submitter, service: service)
    ...
    func testChallengeFlowSuccess() throws {
        ...
        let resultExpectation = expectation(description: "Expect ThreeDS2ActionHandler completion closure to be called.")
        let sut = ThreeDS2CompactActionHandler(adyenContext: Dummy.adyenContext, service: service)
        sut.threeDSRequestorAppURL = URL(string: "http://google.com")
