...
class ThreeDS2CompactActionHandlerTests: XCTestCase {
    ...
    func testChallengeFlowInvalidChallengeToken() throws {
        ...
        sut.handle(mockedAction) { result in
            ...
            case .success:
                XCTFail()
            case let .failure(error):
                guard case .underlyingError(let underlyingError) = error else {
                    XCTFail()
                    return
                }
    ...
    func testFingerprintFlowInvalidEphemeralPublicKey() throws {
        ...
        sut.handle(fingerprintAction) { result in
            ...
            case .success:
                XCTFail()
            case .failure(let actionHandlerError):
                switch actionHandlerError {
                case .underlyingError(let underlyingError):
                    switch underlyingError as? DecodingError{
                    case .dataCorrupted:
                        break
                    default:
                        XCTFail()
